...
public class MainClass01 {
	public static void main(String[] args) throws IOException {
		...
		for(String temp : list) { // 파일목록 반복문 이용 하나씩 가져오기
			...
			// 1970년 1월 1일부터 현재 수정한 날짜를 밀리세컨즈로 반환한다.(TimeStamp)
			long lastMod = f2.lastModified();
			
			// 현재 파일 또는 디렉터리의 마지막 수정한 날짜를 Calendar 객체로 다시 설정
			c.setTimeInMillis(lastMod);
			
			// 마지막 수정한 날짜 포맥변환하여 문자열로 반환
		...
		for(String temp : list) { // 파일목록 반복문 이용 하나씩 가져오기
			...
			// isFile() - 파일이면 true, 파일이 아니면 false
			if(f2.isFile()) {
		 ...
		 *      예) 윈도우 - C:\
		 *          리눅스 - /
		 *          C:\java_lsj\workspace\JavaSE\java.txt
		 *          
		 *  2. 상대경로
		 *      현재 위치 기준
		 *      . -> 현재 위치
		...
		//mkdirs() - 디렉토리 생성
		if(f4.mkdirs()) {
			System.out.println("./upload/"+year+"/"+month+"/"+day + " 디렉터리가 생성되었습니다.");
		}
